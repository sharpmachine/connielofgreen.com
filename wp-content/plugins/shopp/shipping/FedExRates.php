 ...
 *
 * $Id: FedExRates.php 46 2012-07-26 15:21:24Z jdillick $
 **/
...
class FedExRates extends ShippingFramework implements ShippingModule {
	...
	function __construct () {
		...
		$this->wsdl_url = shoppurl(array( 'shopp_fedex'=>'wsdl', 'ver'=>'9' ), 'catalog');
		$this->wsdl();
	...
	function send ($request=false,$url=false,$options=array()) {
		try {
			if ( class_exists('nusoap_client') ) {
				...
				if ( ! $response && ($error = $client->getError()) && SHOPP_DEBUG ) {
					new ShoppError("NuSOAP Error: $error",false,SHOPP_DEBUG_ERR);
					return false;
				}
				mkobject($response);
			} else {
